...
DECLARE
  ...
  v_min_clicks int := 10;
BEGIN
  -- 1) Duplicate add_to_cart_click detection
  WITH clicks AS (
  ...
  -- 2) Missing add_to_cart_success after deduplicated click
  WITH real_clicks AS (
  ...
  IF v_total_clicks >= v_min_clicks AND v_miss_rate >= 5 THEN
    v_miss_severity := CASE WHEN v_miss_rate >= 15 THEN 'high' ELSE 'medium' END;
    INSERT INTO public.monitoring_alerts
      (alert_key, severity, category, title, description, affected_urls,
       suggested_fix, first_detected_at, last_detected_at, is_active, notification_sent)
    VALUES
      (v_miss_key, v_miss_severity, 'analytics',
       ...
       format('%s of %s deduplicated add_to_cart_click events (%s%%) had no add_to_cart_success within %ss in the last hour.',
              v_missing, v_total_clicks, v_miss_rate::text, v_window_seconds),
       (SELECT array_agg(DISTINCT s) FROM unnest(v_miss_examples[1:20]) s),
       'Check ProductDetail.handleAddToCart success path, addItem cart store, and cci-ingest delivery for add_to_cart_success. A silent drop here breaks downstream funnel/CCI reconciliation.',
       now(), now(), true, false)
    ON CONFLICT (alert_key) WHERE is_active
    DO UPDATE SET severity = EXCLUDED.severity,
                  description = EXCLUDED.description,
