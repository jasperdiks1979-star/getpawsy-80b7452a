...
CREATE OR REPLACE FUNCTION public.canonical_ingest_recent(hours int DEFAULT 2)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH inserted AS (
    ...
    WHERE e.created_at >= since
      AND e.event_name IN ('product_view','product_card_click','add_to_cart_click','add_to_cart_success',
...
REVOKE ALL ON FUNCTION public.canonical_ingest_recent(int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.canonical_ingest_recent(int) TO service_role;
