...
-- Pinterest Destination Guard: SECURITY DEFINER admin functions
-- Owner-privileged so RLS doesn't block legitimate cleanup.
...
BEGIN
  ...
  -- 2) Cancel pending queue items targeting ineligible products
  FOR r IN
    WITH cancelled AS (
...
REVOKE ALL ON FUNCTION public.pinterest_guard_sweep() FROM public, anon;
GRANT EXECUTE ON FUNCTION public.pinterest_guard_sweep() TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.pinterest_guard_audit() FROM public, anon;
GRANT EXECUTE ON FUNCTION public.pinterest_guard_audit() TO authenticated, service_role;
