ALTER TABLE public.cinematic_ad_jobs
  ...
  ADD COLUMN IF NOT EXISTS render_priority_score integer DEFAULT 0,
...
CREATE OR REPLACE FUNCTION public.claim_cinematic_ad_job(p_worker_id text, p_job_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, product_id uuid, product_slug text, hook_variant text, scene_assets jsonb, vo_url text, music_url text, render_token text, render_attempts integer, previous_status text, render_worker_id text, preset text, hook_text text, subhook_text text, cta_text text, product_name text, product_price numeric, pin_title text, pin_description text, pin_destination_url text, hashtags text[], vo_script text, product_lock boolean, validation_report jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SET
    ...
    render_started_at = CASE WHEN j.status = 'rendering' THEN j.render_started_at ELSE now() END,
