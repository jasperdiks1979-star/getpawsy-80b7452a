...
import { detectNiche, getStyleDNA, type NicheKey } from "../_shared/pinterest-style-dna.ts";
...
async function seedMissingMediaJobs(sb: Sb, limit: number, source: string) {
  const { data: pins, error } = await sb
    ...
    .select("id, product_id, product_slug, product_name, status, priority, created_at, pin_image_url, meta")
  ...
  const rows = (pins ?? []).map((p: any, i: number) => ({
    ...
    source,
    prompt: { queue_status: p.status, queue_priority: p.priority, seeded_from_meta: p.meta ?? {} },
  }));
...
async function inventory(sb: Sb) {
  const [{ data: queueRows }, { data: jobs }, { data: settings }] = await Promise.all([
    ...
    sb.from("pinterest_creative_factory_jobs").select("status,stage,media_url", { count: "exact" }),
    sb.from("pinterest_creative_factory_settings").select("*").eq("id", 1).maybeSingle(),
  ]);
  const q = queueRows ?? [];
  const j = jobs ?? [];
  const missing = q.filter((r: any) => !r.pin_image_url).length;
  const readyPins = q.filter((r: any) => r.pin_image_url && ["queued", "draft"].includes(r.status)).length;
  const readyMedia = j.filter((r: any) => r.media_url && r.status === "completed").length;
  const readyPrompts = j.filter((r: any) => ["planned", "rendering", "quality_control", "completed"].includes(r.stage)).length;
  return {
...
async function leaseJobs(sb: Sb, limit: number, owner: string) {
  ...
  for (const job of candidates ?? []) {
    ...
    const { data, error: updErr } = await sb
      .from("pinterest_creative_factory_jobs")
      .update({ status: "running", lease_owner: owner, leased_until: until, attempt_count: Number(job.attempt_count ?? 0) + 1 })
      .eq("id", job.id)
      .in("status", ["pending", "retry"])
      .select("*")
      .maybeSingle();
...
async function generateImage(prompt: string, productImageUrl: string | null, model: string, metrics: Record<string, unknown>) {
...
async function validateWithAi(bytes: Uint8Array, prompt: string, metrics: Record<string, unknown>) {
  if (!LOVABLE_API_KEY) return { ok: true, score: 74, reasons: ["ai_qc_skipped_no_key"], notes: "deterministic fallback" };
  const started = Date.now();
  ...
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: "text", text: `Score this Pinterest creative from 0-100 for premium pet ecommerce quality, product truth, mobile-safe composition, and non-spammy overlay. Return strict JSON: {"score":number,"ok":boolean,"reasons":[string],"notes":"string"}. Prompt intent: ${prompt.slice(0, 1200)}` },
          ],
        }],
        temperature: 0.1,
      }),
    ...
    if (!resp.ok) return { ok: true, score: 70, reasons: [`ai_qc_http_${resp.status}`], notes: "soft-pass; deterministic gates still applied" };
    ...
    return { ok: Boolean(verdict.ok ?? score >= 70), score, reasons: Array.isArray(verdict.reasons) ? verdict.reasons : [], notes: String(verdict.notes ?? "") };
  } catch (e) {
    metrics.ai_quality_latency_ms = Date.now() - started;
    return { ok: true, score: 70, reasons: [`ai_qc_error:${e instanceof Error ? e.message.slice(0, 80) : "unknown"}`], notes: "soft-pass; deterministic gates still applied" };
  }
}
...
function deterministicQuality(copy: any, bytes: Uint8Array) {
  ...
  if (overlayWords < 2 || overlayWords > 5) reasons.push("overlay_word_count_invalid");
  if (!String(copy.title ?? "").trim()) reasons.push("title_missing");
  if (!String(copy.description ?? "").includes("getpawsy.pet")) reasons.push("description_missing_getpawsy_destination");
  const score = Math.max(0, 82 - reasons.length * 20);
  return { ok: reasons.length === 0, scores: { total: score, deterministic: score, visual_quality_score: score, mobile_safety_score: score }, reasons, notes: "factory_fast_gate" };
}
  ...
  try {
    ...
    if (pin.pin_image_url) {
      await sb.from("pinterest_creative_factory_jobs").update({
        status: "completed", stage: "queue", media_url: pin.pin_image_url, completed_at: new Date().toISOString(), metrics,
      }).eq("id", job.id);
      return { ok: true, skipped: "already_has_media" };
    }

    const { data: product, error: pErr } = await timed("product_lookup", metrics, async () => sb
      .from("products")
      .select("id, name, slug, description, category, product_type, image_url, key_feature, benefit_angle, description_bullets, price, is_active")
    ...
    if (!validation.valid) throw new Error(`copy_validation_failed:${validation.errors.join(",")}`);

    const prompt = buildPrompt(product, niche, copy.overlay, job);
    await timed("planning", metrics, async () => {
      await sb.from("pinterest_creative_factory_jobs").update({ stage: "planned", prompt: { text: prompt, niche, copy } }).eq("id", job.id);
      return true;
    });
    ...
    const qc = await timed("quality_control", metrics, async () => {
      ...
      return {
        ...
        scores: { ...fast.scores, total: Math.round(ai.score), ai_visual: Math.round(ai.score) },
        reasons: ai.reasons ?? [],
        notes: ai.notes ?? "",
      };
    });
    ...
    if (!integrity.passed) throw new Error(`integrity_guard_failed:${integrity.blocking_reasons.join(",")}`);
    ...
    await timed("queue_attach", metrics, async () => {
      const meta = {
        ...
        intelligence: { scores: qc.scores, niche_key: niche, model: settings?.model ?? DEFAULT_MODEL },
      };
...
async function work(sb: Sb, requestedLimit: number) {
  const { data: settings } = await sb.from("pinterest_creative_factory_settings").select("*").eq("id", 1).maybeSingle();
  if (settings && settings.enabled === false) return { ok: false, message: "factory_disabled" };
  const owner = `factory-${traceId()}`;
  const limit = Math.max(1, Math.min(Number(requestedLimit || settings?.max_jobs_per_run || 1), Number(settings?.max_jobs_per_run || 3), 5));
...
Deno.serve(async (req) => {
  ...
  try {
    if (action === "status") return json({ ok: true, traceId: traceId(), inventory: await inventory(sb) });
    ...
    if (action === "stress_test") {
      ...
      return json({
        ...
        simulation: {
          ...
          stage_split: ["discovery", "planning", "image_generation", "quality_control", "storage_upload", "queue_attach"],
          max_concurrency: inv.settings?.max_concurrency ?? 1,
          max_jobs_per_run: inv.settings?.max_jobs_per_run ?? 3,
          expected_runs_for_1000: Math.ceil(queuedJobs / Math.max(1, Number(inv.settings?.max_jobs_per_run ?? 3))),
          deadlock_risk: "low: leases expire and jobs resume from persisted stage",
          permanent_growth_risk: "bounded: unique pin_queue_id and retry/failed terminal state",
        },
        inventory: inv,
      });
    }
    return json({ ok: false, traceId: traceId(), message: `unknown_action:${action}` }, 400);
  } catch (e) {
    return json({ ok: false, traceId: traceId(), message: e instanceof Error ? e.message : String(e) }, 500);
  }
});