...
async function visualScore(args: {
  ...
  const sys =
    ...
    "must score below 50 on pinterest_native. " +
  ...
  const tools = [
    {
      ...
      function: {
        name: "rate_pin",
        description: "Rate a Pinterest pin on three visual axes.",
        parameters: {
          type: "object",
          properties: {
          ...
          required: ["visual_balance", "readability", "pinterest_native", "notes"],
  ...
  try {
    ...
    return { visual_balance: 70, readability: 70, pinterest_native: 70, notes: `error:${(e as Error).message.slice(0, 60)}` };
  }
}
...
export async function scorePin(args: {
  ...
  const total =
    0.20 * det.mobile_safety +
    0.20 * vis.visual_balance +
    0.20 * vis.readability +
    0.20 * det.viral_potential +
    0.20 * vis.pinterest_native;
