// pinterest-video-publisher — REMOVED (P0 architecture remediation 2026-06-26).
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await sb.from("guardian_publish_gate_log").insert({
      pipeline: "pinterest-video-publisher",
      decision: "deny",
      reason: "legacy_video_publisher_removed",
      guardian_color: "red",
      guardian_score: 0,
      meta: { caller },
    });
  ...
  return new Response(
    JSON.stringify({
      ...
      message: "pinterest-video-publisher has been retired. Enqueue into pcie2_publish_queue; pcie2-publisher is the only canonical Pinterest publisher.",
