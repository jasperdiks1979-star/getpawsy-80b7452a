...
async function sbQuery(table: string, params: string): Promise<unknown[]> {
  ...
  if (!r.ok) {
    console.error(`[sitemap-v2] REST error ${table}:`, r.status);
    return [];
  }
  return await r.json();
}
...
async function guidesSitemap(today: string): Promise<string> {
  ...
  console.log(`[SITEMAP-V2] GUIDES SITEMAP SERVED: ${guides.length} URLs`);
...
function staticSitemap(today: string): string {
  ...
  const urls = pages
    ...
    .join("\n");
  console.log(`[SITEMAP-V2] STATIC SITEMAP SERVED: ${pages.length} URLs`);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>`;
}
...
async function productsSitemap(today: string): Promise<string> {
  const products = (await sbQuery(
    "products",
    "select=id,name,slug,updated_at,image_url&is_active=eq.true&is_duplicate=eq.false&order=updated_at.desc"
  )) as Array<{ id: string; name: string; slug: string | null; updated_at: string; image_url: string | null }>;
  const now = Date.now();
  let urls = "";
  for (let i = 0; i < products.length; i++) {
    ...
    const pri = Math.min(
      0.95,
      (i < 100 ? 0.9 : i < 500 ? 0.8 : 0.7) + (days <= 1 ? 0.05 : days <= 7 ? 0.02 : 0)
    ).toFixed(2);
  ...
  console.log(`[SITEMAP-V2] PRODUCTS SITEMAP SERVED: ${products.length} URLs`);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">\n${urls}\n</urlset>`;
}

async function categoriesSitemap(today: string): Promise<string> {
  const cats = (await sbQuery("categories", "select=slug,name")) as Array<{ slug: string; name: string }>;
  const prods = (await sbQuery("products", "select=category&is_active=eq.true&is_duplicate=eq.false")) as Array<{
  ...
  for (const p of prods) {
    ...
    const sl = toSlug(p.category);
    if (seen.has(sl)) continue;
    seen.add(sl);
    urls += `\n  <url>\n    <loc>${BASE_URL}/products?category=${sl}</loc>\n    <lastmod>${today}</lastmod>\n    <changefreq>daily</changefreq>\n    <priority>0.7</priority>\n  </url>`;
  }
  console.log(`[SITEMAP-V2] CATEGORIES SITEMAP SERVED: ${seen.size} URLs`);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>`;
}

async function bestsellersSitemap(today: string): Promise<string> {
  const data = (await sbQuery("bestsellers", "select=slug,updated_at&is_active=eq.true")) as Array<{
    slug: string;
    updated_at: string;
  }>;
  let urls = "";
  ...
  console.log(`[SITEMAP-V2] BESTSELLERS SITEMAP SERVED: ${data.length} URLs`);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>`;
}

async function collectionsSitemap(today: string): Promise<string> {
  const data = (await sbQuery("seo_collections", "select=slug,updated_at&is_active=eq.true")) as Array<{
    slug: string;
    updated_at: string;
  }>;
  let urls = "";
  ...
  console.log(`[SITEMAP-V2] COLLECTIONS SITEMAP SERVED: ${data.length} URLs`);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>`;
}
...
async function blogSitemap(today: string): Promise<string> {
  const data = (await sbQuery(
    "blog_posts",
    "select=slug,title,published_at,featured_image&is_published=eq.true&order=published_at.desc"
  )) as Array<{ slug: string; title: string; published_at: string; featured_image: string | null }>;
  let urls = "";
  for (const p of data) {
    const lm = p.published_at?.split("T")[0] || today;
  ...
  console.log(`[SITEMAP-V2] BLOG SITEMAP SERVED: ${data.length} URLs`);
  return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">\n${urls}\n</urlset>`;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.error("[sitemap-v2] error:", e);
    return new Response(
