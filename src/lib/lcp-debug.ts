...
function updateOverlay() {
  ...
  const lines = [
    ...
    `<span style="color:#ff0">Component mounted: ${formatMs(gt.componentMountedAt)}</span>`,
    `<span style="color:#ff0">Fetch initiated: ${formatMs(gt.productsFetchInitiatedAt)} (gate: ${gt.productsFetchGateReason})</span>`,
    ...
    `<span style="color:#0f0;font-weight:bold">Grid 1st item VISIBLE: ${formatMs(gt.gridFirstItemVisibleAt)}</span>`,
    ...
    `navStart → component mount: ${formatMs(gt.componentMountedAt)}`,
    `navStart → fetch initiated: ${formatMs(gt.productsFetchInitiatedAt)}`,
    ...
    `navStart → 1st VISIBLE: ${formatMs(gt.gridFirstItemVisibleAt)}`,
    ...
    copyBtn.onclick = () => {
      const json = JSON.stringify({
        ...
        fetchInstrumentation: {
          componentMountedAt: gt.componentMountedAt,
          productsFetchInitiatedAt: gt.productsFetchInitiatedAt,
          productsFetchGateReason: gt.productsFetchGateReason,
          productsLoadStartAt: gt.productsLoadStartAt,
          productsLoadEndAt: gt.productsLoadEndAt,
          fetchInitiationDelay: gt.productsFetchInitiatedAt !== null ? Math.round(gt.productsFetchInitiatedAt) : null,
        },
