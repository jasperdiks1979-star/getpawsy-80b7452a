...
const metaTemplates = {
  ...
  premium: (productName: string, features: string, price?: number) =>
    `Premium ${productName} with ${features}. ${price ? `Starting at $${price.toFixed(2)}. ` : ''}Eco-friendly & hand-selected. Shop GetPawsy!`,
};
...
export function generateCategoryMetaDescription(categoryName: string): string {
  ...
  const categoryDescriptions: Record<string, string> = {
    dog: 'Discover premium dog supplies at GetPawsy. From cozy beds to durable toys, we have everything your furry friend needs. Premium quality, eco-friendly products with free shipping on eligible orders over $35.',
