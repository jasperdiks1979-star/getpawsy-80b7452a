...
const picks = [
  { key: 'bestOverall' as const, label: 'Best Overall', accentColor: 'text-primary', badgeBg: 'bg-primary/10 ring-primary/20', numberBg: 'bg-primary' },
  { key: 'bestBudget' as const, label: 'Best Budget', accentColor: 'text-green-600', badgeBg: 'bg-green-500/10 ring-green-500/20', numberBg: 'bg-green-600' },
  { key: 'bestPremium' as const, label: 'Best Premium', accentColor: 'text-amber-600', badgeBg: 'bg-amber-500/10 ring-amber-500/20', numberBg: 'bg-amber-600' },
] as const;

/** Only returns true if the pick has a real image, valid name, and a /product link */
function isValidPick(pick: { name: string; link: string; image?: string }): boolean {
  if (!pick.name || !pick.link) return false;
  if (!pick.image || pick.image.startsWith('/images/guides/')) return false;
  if (!pick.link.startsWith('/product')) return false;
  return true;
}
  ...
  return (
    <div className="relative mb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {validPicks.map(({ key, label, accentColor, badgeBg, numberBg }, index) => {
          const pick = data[key];
          return (
            <Link
              key={key}
