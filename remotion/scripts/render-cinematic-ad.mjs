...
async function main() {
  ...
  // Phase 3: route ANY job that has a real motion_storyboard (>=4 scenes,
  // generated by cinematic-motion-engine) through the Remotion compositor so
  // we get parallax / depth / camera moves instead of the ffmpeg zoompan
  // slideshow. Falls back to ffmpeg only when both checks fail.
  ...
  if (REMOTION_TYPES.has(job.content_type) || hasMotionStoryboard) {
    ...
    const remotionResult = await new Promise((resolve) => {
      ...
      p.on("error", (err) => resolve({ ok: false, code: -1, err }));
    });
    if (remotionResult.ok) {
      console.log(`[render] remotion cinematic render complete job=${job.job_id}`);
      return;
    }
    // Fallback to ffmpeg slideshow so a Remotion crash never fails the whole job.
    console.warn(`[render] remotion renderer exited code=${remotionResult.code} — falling back to ffmpeg zoompan`);
  }
